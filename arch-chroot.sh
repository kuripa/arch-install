...
configure_user() 
{
    ...
    password_root=$(dialog --stdout --passwordbox "Enter new root password" 0 0) || exit 1
    clear
    : ${password_root:?"password cannot be empty"}

    password2_root=$(dialog --stdout --passwordbox "Enter root password again" 0 0) || exit 1
    clear
    ...
    password=$(dialog --stdout --passwordbox "Enter password for user $username" 0 0) || exit 1
    clear
    : ${password:?"password cannot be empty"}

    password2=$(dialog --stdout --passwordbox "Enter password again for user $username" 0 0) || exit 1
    clear
...
install_essential_packages()
{
  ...
  pacman -S firefox libreoffice thunderbird --noconfirm
